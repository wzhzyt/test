...
function Bubbles(container, self, options) {
  ...
  //接受一个 JSON 对象并创建对应的聊天气泡,here为当前状态
  ...
  this.reply = function(turn) {
    ...
    //questionsHTML，用于存储生成的回复选项按钮的 HTML
    questionsHTML = ""
    if (!turn) return
  ...
  // create a bubble
  //创建并显示聊天气泡
  var bubbleQueue = false //用于跟踪添加气泡的队列
  var addBubble = function(say, posted, reply, live) {
    if (say.includes('<div class="card">')) {
      // If the reply contains the card content, insert it directly without creating a bubble
      var cardContent = document.createElement("div");
      cardContent.innerHTML = say;
      bubbleWrap.insertBefore(cardContent, bubbleTyping);
      posted();
      return; // Exit the function since we've already inserted the card content
    }
    ...
    if (reply !== "") {
      ...
      bubble.addEventListener("click", function(e) {
        if (e.target.classList.contains('bubble-button')) {
            var inputWraps = document.getElementsByClassName("input-wrap");
            if (inputWraps.length > 0) {
                var firstInputWrap = inputWraps[0];
                var textareas = firstInputWrap.getElementsByTagName("textarea");
                if (textareas.length > 0) {
                    var replyContent = e.target.innerText; // 获取点击的按钮文本
                    textareas[0].value = "";
                    var textarea = textareas[0];
                    animateText(replyContent, textarea);
                }
            }
        }
      }
